...
function isConfig(c: any): c is Config {
	...
	return port && typeof port == 'number' && port < 65535 && port > -1 && 
		...
		logs && (logs.level === null || logs.level === 'off' || (['simple', 'verbose'].includes(logs.level) && typeof logs.savetofile === 'boolean' && typeof logs.extension === 'string' &&
...
if (config.templater.enabled && (!fs.existsSync(config.templater.views) || !fs.lstatSync(config.templater.views).isDirectory())) log.warn('Views path invalid/doesn\'t exist. Your site serves no pages');
